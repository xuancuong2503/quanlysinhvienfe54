...
var luuLocalStorage = function(){
    ...
    // dem mang chuoi sinh vien luu vao localStrogae
    localStorage.setItem('mangSinhVien',sMangSinhVien);
}   

var chinhSua = function(maSV){
    document.querySelector('#maSinhVien').disabled=true;
    //từ mã sinh viên tìm sinh viên trong mảng mangSinhVien
    for(var index =0;index<mangSinhVien.length;index++){
        //mỗi lần duyệt mảng lấy ra 1 đốit tượng sinh viên
        var sv=mangSinhVien[index];

        //so sánh nếu masv truyền vào=== với đối tượng đang duyệt =>gắn ngược lại lên các control phía trên
...
//viet phuong thuc lay du lieu tu local storage-> sau khi nguoi dung vua vao trang web
var layMangSinhVienStorage =function(){
    // kiem tra du lieu co trong localstorage khong
    if(localStorage.getItem('mangSinhVien')){
        //lay du lieu duoc luu trong localstroae ra ngoai
        var sMangSinhVien = localStorage.getItem('mangSinhVien');
        // bien du lieu tu chuoi ve object js gan vao mangSinhVien
...
document.querySelector('#btnLuuThongTin').onclick=function(){
    //lấy thông tin người dùng sau khi thay đổi gắn vào đối tượng sinhvien
    var sv = new SinhVien();
    sv.maSinhVien=document.querySelector('#maSinhVien').value;
    sv.tenSinhVien=document.querySelector('#tenSinhVien').value;
    sv.loaiSinhVien=document.querySelector('#loaiSinhVien').value;
    ...
    // tìm trong mảng sinh viên đối tượng cùng mã =>cập nhật giá trị
    for(var index =0;index<mangSinhVien.length;index++){
        var sinhVienCapNhat =mangSinhVien[index];
        // tìm ra sinhvien trong mảng có mã = với mã sinh viên trên giao diện=> cập nhật giá trị
